        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-muted-foreground mb-8 max-w-2xl">
              Quản lý kho video 360 và điều khiển sự kiện VR một cách chuyên nghiệp. 
              Tạo trải nghiệm tương tác đỉnh cao cho người dùng.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to="/events/create">
                <Button size="lg" className="bg-gradient-primary hover:opacity-90 shadow-vr">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat, index) => {
            ...
            return (
              <Card key={index} className="bg-vr-surface border-border shadow-card hover:shadow-vr transition-all duration-300">
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className={`w-3 h-3 rounded-full ${
                      event.status === 'active' ? 'bg-vr-secondary' :
                      event.status === 'scheduled' ? 'bg-vr-primary' : 'bg-muted-foreground'
                    }`} />
                    <div>
                      <h4 className="font-medium text-foreground">{event.name}</h4>
                      <p className="text-sm text-muted-foreground">
                        ID: {event.id} • {event.participants} người tham gia
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
